...
The Finite-Time Lyapunov Exponent (FTLE) field is a fundamental tool in fluid dynamics, dynamical systems, and transport phenomena, enabling the identification of Lagrangian Coherent Structures (LCSs) [@Haller_annual_review]. Also, the FTLE play a crucial role in elucidating transport mechanisms and identifying separatrices in dynamic fluid flows [@Brunton_LCS]. This work presents a Python-based solver for computing FTLE fields from velocity data, featuring optimized integration techniques and parallelized computations using multiprocessing.
...
Understanding transport barriers and coherent structures in unsteady flows is crucial in various scientific and engineering domains, such as oceanography, meteorology, and aerodynamics. Despite the importance of FTLE computations, existing implementations are often specialized, computationally expensive, or lack extensibility.
...
- Allowing for an easy extension of different types of file formats to read the velocity and grid data.
- Leveraging Numba for computational efficiency in particle advection.
- Enabling parallelized FTLE computations using multiprocessing for improved performance.


# Implementation

Initially, a grid of particles $X_{0} \subset \mathbb{R}^{2}$, is established across the domain of interest. These particles are integrated in the velocity field from the initial time of $0$ to the final time $T$, yielding a time-T particle flow map denoted as $\Phi_{0}^{T}$, and defined as follows:

$$
\Phi_{0}^{T}: \mathbb{R}^{2} \to \mathbb{R}^{2}; \mathbf{x}(0) \mapsto \mathbf{x}(0) + \int_{o}^{T} \mathbf{u}(\mathbf{x}(\tau), \tau)  d\tau
$$

Where $\mathbf{u}(\mathbf{x}(\tau), \tau)$ denotes the time-dependent velocity field over the particle trajectory $\mathbf{x}(\tau)$ at a time $\tau$. The flow map jacobian $\mathbf{D} \Phi_{0}^{T}$ is then computed by a central finite difference of the neighbouring particles in a Cartesian mesh such as:
...
Where $x$ and $y$ denotes the coordinates of the particles and the subscripts $i$ and $j$ denotes their indices in the computational domain. Finally the Cauchy-Green deformation tensor is computed as:

$$
\mathbf{\Delta} = (\mathbf{D} \Phi_{0}^{T})^{\ast} \mathbf{D} \Phi_{0}^{T}
$$
...
$$
\sigma (\mathbf{D} \Phi_{0}^{T} ; \mathbf{x_{0}}) = \frac{1}{\lvert T \rvert} \log{ \sqrt{\lambda_{max}(\Delta (\mathbf{x_{0}}))}}
$$

The FTLEs are computed using an auxiliary grid in which the LES properties are interpolated on. Fig. \autoref{fig:numerical_method} presents an example of the spatial discretization approach used in the present study. A reference mesh (indicated by the red dots in Fig. \autoref{fig:numerical_method}) is first placed on top of the LES grid and the overlapping points on the airfoil solid surface are removed. The auxiliary grid (represented by the black dots) is then constructed with the maximum distance allowed from the reference points to the airfoil surface ensuring that all points remain outside the solid body, as shown in the inset plot. For each different initial condition this process is repeated with the reference mesh created yielding a square field of view of $\pm 1.5 C$ centered in the airfoil quarter-chord frame of reference, indicated by the black arrows. The auxiliary points are then integrated along the flow using a second-order Adams-Bashforth/fourth order Runge-Kutta/euler method. At each integration step, the velocity field in the particles position is determined by interpolation of the LES data. Different interpolation schemes were implemented and the code is structured for easy implementation of additional schemes.

![Auxiliary grid used to compute the flow map Jacobian. The properties at the reference point marked in red are computed by integrating then performing the central finite difference of the auxiliary grid points in black.\label{fig:numerical_method}](fig1.png)

The FTLE fields can be computed by integrating the particles forward or backwards in time. This choice yields different interpretations of the LCSs providing analogs for stable (forward time integration) and unstable manifolds (backwards time integration) from dynamical systems [@Brunton_LCS; @Haller_annual_review]. In the present work we employ the backward time integration of the particles as it enables a direct measure of material transport in forward time, mimicking experimental flow visualization by tracers. This choice is justified since we are most interested in visualizing the different interaction from the vortices, shear layers and the wake patters associated with this particular flow configuration, and which are all a well characterized by the ridges of the backward time FTLE field. An exemple of the backward-time FTLE field is provided in the figure \autoref{fig:example}.

![Backward time FTLE field of moving airfoil.\label{fig:example}](fig2.png)
...
We acknowledge Fundação de Amparo à Pesquisa do Estado de São Paulo (FAPESP) under Grants No. 2013/08293-7, 2013/07375-0, 2021/06448-0, 2022/08567-9 and 2022/09196-4 for funding this project.
